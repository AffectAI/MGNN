...
class DepressionDataset(BaseActionDataset):
    ...
    def __init__(self,
        ...
        self.depression_norm = depression_norm
        self.MAX_BDI2_VALUE = 63
        super().__init__(
            ann_file,
            pipeline=pipeline,
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for line in fin:
            ...
            if self.label_type == 'int':
                ...
                label = [float(x) for x in line_split[idx:]]
            else:
                label = [int(x) for x in line_split[idx:]]

            # norm label to [0, 1]
            if self.depression_norm:
                label = [x/self.MAX_BDI2_VALUE for x in label]
