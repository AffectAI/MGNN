...
@TRANSFORMS.register_module()
class SampleAffWildFrames(SampleFrames):
    ...
    def _get_clips(self, center_index, skip_offsets, shot_info):
        """Get clip offsets."""
        start = center_index - (self.clip_len - 1) * self.frame_interval
        end = center_index + 1
        frame_inds = list(range(start, end, self.frame_interval))
        if not self.test_mode:
            frame_inds = list(frame_inds[:-1] + skip_offsets) + [frame_inds[-1]]
        frame_inds = np.clip(frame_inds, shot_info[0], shot_info[1] - 1)
